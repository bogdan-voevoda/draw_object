...
//float PCF(in vec4 v_vert_smcoord) {
//    float res = 0.0;

//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2(-1,-1));
//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2( 0,-1));
//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2( 1,-1));
//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2(-1, 0));
//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2( 0, 0));
//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2( 1, 0));
//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2(-1, 1));
//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2( 0, 1));
//    res += shadow2D(depthTexture, v_vert_smcoord, ivec2( 1, 1));
...
void main(void) {
    ...
    float shadow  = float(v_vert_smcoord.z <= texture2D(depthTexture, v_vert_smcoord.xy).x);
    ...
    gl_FragColor = (u_material_emission + ambient + diff + spec) * texture2D(u_material_texture, v_vert_texcoord) * shadow;
}
